...
export const papers: Paper[] = [
  {
    ...
    cards: {
      plain: {
        ...
        markdown: "# 为什么差分注意力能过滤噪声\n\n这篇论文可以理解为给注意力机制加了一组对照信号，让模型更容易削弱无关上下文。\n\n- 核心直觉是两路注意力相减\n- 适合解释长上下文里的干扰项\n- 局限在于训练和推理成本需要继续验证\n\n## 直观理解\n\n普通注意力像一次性把所有上下文都扫进去，差分注意力更像同时看正证据和反证据，再把不稳定的部分抵消掉。"
      },
      expert: {
        kind: "expert",
        ...
        markdown: "# 专家阅读：机制、证据与可疑点\n\n专家视角重点关注差分结构是否真的带来因果性改进，以及 ablation 是否排除了参数量因素。\n\n## 需要核对的问题\n\n- 是否有同参数量 baseline\n- 注意力热图是否只是后验解释\n- 长上下文收益是否来自数据或训练配方"
      },
      reproduction: {
        kind: "reproduction",
        title: "最小可行复现路径",
        ...
        markdown: "# 最小可行复现路径\n\n先复现一个小型语言模型上的差分注意力层，再比较困惑度和长上下文 QA。\n\n```bash\npython train.py --attention differential --context 4096\n```\n\n| 阶段 | 目标 |\n| --- | --- |\n| sanity check | 模块能训练 |\n| benchmark | 长上下文干扰评测 |"
      }
    }
  },
  {
    ...
    cards: {
      plain: {
        ...
        markdown: "# 把函数映射理解成连续版神经网络\n\n这类方法不是预测一个点，而是学习从一个函数到另一个函数的整体变换。\n\n- 适合 PDE 和物理仿真\n- 积分核像可学习的连续卷积\n- 泛化依赖采样分辨率"
      },
      expert: {
        kind: "expert",
        title: "算子学习的假设边界",
        ...
        markdown: "# 算子学习的假设边界\n\n专家阅读关注算子泛化是否来自结构先验，还是来自数据分布相对稳定。\n\n> 关键问题是跨网格泛化是否真的成立。"
      },
      reproduction: {
        kind: "reproduction",
        title: "PDE 小任务复现计划",
        ...
        markdown: "# PDE 小任务复现计划\n\n选择 Burgers 方程数据集，用低分辨率训练，高分辨率测试来验证核心说法。\n\n- 准备公开 PDE 数据\n- 实现积分核层\n- 报告跨分辨率误差"
      }
    }
  }
];
